...
if __name__ == "__main__":
    ...
    train_questions_file=config.data_root + "test_question.txt"  
    qsns = []
    ...
    train_tables_list = os.listdir(config.test_tables)
    
    w2v_feature_train = csv.writer(open("w2v_col_q_pair_test_new.csv", 'w'), delimiter=',',lineterminator='\n')
    ...
    for x,train_table in enumerate(train_tables_list):
        ...
        table_index = int(train_table.split('.')[0])
 
        train_table_filename = config.test_tables + str(table_index) + ".csv"
        ...
        if True: #table_index == 40: #table_index != 221 and table_index != 24 and table_index != 77:
            ...
            column_header_list = process_column(config.test_tables, table_index, include_header = False, include_contents = True)
